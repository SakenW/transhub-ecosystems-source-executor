"""Private stdin/stdout bridge for the trusted Obsidian ZIP host.

The bridge accepts one bounded JSON request on stdin, keeps the ZIP in memory,
and returns only the Adapter's normalized source catalog.  It never accepts a
path, URL, command, credential, or output location from the caller.
"""
...
from .public_zip_closure import (
    MAX_ZIP_BYTES,
...
BRIDGE_PROTOCOL: Final = "trans-hub.obsidian-zip-bridge.v1"
PUBLIC_EXECUTOR_PROTOCOL: Final = "trans-hub.obsidian-public-executor.v1"
PUBLIC_RESULT_PROTOCOL: Final = "trans-hub.public-discovery-result"
PUBLIC_RESULT_REVISION: Final = 1
MAX_REQUEST_BYTES: Final = ((MAX_ZIP_BYTES + 2) // 3) * 4 + 1_024
...
class ObsidianZipBridgeError(ValueError):
    """A deterministic protocol failure safe to expose to the trusted host."""


def handle_request(raw: bytes) -> bytes:
    """Execute one bridge request without reading files or environment state."""
    ...
    if not isinstance(request, dict) or set(request) != {
        ...
        raise ObsidianZipBridgeError("obsidian_zip_bridge_request_invalid")
    if request["protocol"] != BRIDGE_PROTOCOL:
        raise ObsidianZipBridgeError("obsidian_zip_bridge_protocol_invalid")
    encoded = request["zip_base64"]
    ...
    try:
        ...
        raise ObsidianZipBridgeError("obsidian_zip_bridge_zip_base64_invalid") from exc
    components = components_from_zip(zip_bytes)
    ...
    if len(response) > MAX_RESPONSE_BYTES:
        raise ObsidianZipBridgeError("obsidian_zip_bridge_response_too_large")
    return response
...
def handle_public_request(raw: bytes) -> bytes:
    ...
    if not isinstance(request, dict) or set(request) != {
        ...
        raise ObsidianZipBridgeError("obsidian_public_executor_request_invalid")
    if request["protocol"] != PUBLIC_EXECUTOR_PROTOCOL:
        raise ObsidianZipBridgeError("obsidian_public_executor_protocol_invalid")
    ...
    if not isinstance(materialization_target_digest, str) or not _DIGEST.fullmatch(
        ...
        raise ObsidianZipBridgeError("obsidian_public_executor_target_invalid")
    encoded = request["zip_base64"]
    policy_revision = request["policy_revision"]
    if (
        not isinstance(encoded, str)
...
def _validate_public_manifest_fields(
    components: tuple[PublicZipComponent, ...],
) -> None:
    ...
    if manifest_component is None:
        raise ObsidianZipBridgeError("obsidian_public_executor_manifest_invalid")
...
def validate_public_source_catalog(value: object) -> None:
    ...
    if not isinstance(value, dict) or set(value) != {
        ...
        raise ObsidianZipBridgeError("obsidian_public_executor_catalog_invalid")
    if (
...
def _adapter_snapshot(
    closure: tuple[PublicZipComponent, ...],
) -> dict[str, object]:
    ...
    if not isinstance(snapshot, dict) or not isinstance(
        ...
        raise ObsidianZipBridgeError("obsidian_zip_bridge_adapter_output_invalid")
...
def _parse_unique_json(raw: bytes) -> object:
    def unique_object(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in result:
                raise ObsidianZipBridgeError("obsidian_zip_bridge_json_duplicate_key")
            result[key] = value
        return result
    ...
    except ObsidianZipBridgeError:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ObsidianZipBridgeError("obsidian_zip_bridge_json_invalid") from exc
...
def _canonical_text(value: object) -> str:
    ...
    if isinstance(value, int):
        if abs(value) > _MAX_SAFE_INTEGER:
            raise ObsidianZipBridgeError("obsidian_public_executor_json_invalid")
        return str(value)
    if isinstance(value, str):
        try:
            value.encode("utf-8", errors="strict")
        except UnicodeEncodeError as exc:
            raise ObsidianZipBridgeError("obsidian_public_executor_json_invalid") from exc
...
def _error_response(code: str, protocol: str = BRIDGE_PROTOCOL) -> bytes:
    safe_code = code if _ERROR_CODE.fullmatch(code) else "obsidian_zip_bridge_failed"
...
def main() -> None:
    _main(handle_request, BRIDGE_PROTOCOL)
...
def _main(handler: Callable[[bytes], bytes], error_protocol: str) -> None:
    ...
    if len(raw) > MAX_REQUEST_BYTES:
        sys.stdout.buffer.write(
            _error_response("obsidian_zip_bridge_request_too_large", error_protocol)
        )
        raise SystemExit(2)
    try:
        ...
        PublicZipClosureError,
        ObsidianZipBridgeError,
    ) as exc:
        sys.stdout.buffer.write(_error_response(str(exc), error_protocol))
        raise SystemExit(2) from None
    except Exception:
        sys.stdout.buffer.write(
            _error_response("obsidian_zip_bridge_failed", error_protocol)
        )
        raise SystemExit(2) from None
